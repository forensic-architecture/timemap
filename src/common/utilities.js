...
let { DATE_FMT, TIME_FMT } = appConfig;
...
export function keyFromEnv(ext) {
  return appConfig[ext];
}

export function urlFromEnv(ext) {
  if (appConfig[ext]) {
    if (!Array.isArray(appConfig[ext])) {
      return [`${appConfig.SERVER_ROOT}${appConfig[ext]}`];
    } else {
      return appConfig[ext].map(
        (suffix) => `${appConfig.SERVER_ROOT}${suffix}`,
      );
    }
  } else {
    return null;
